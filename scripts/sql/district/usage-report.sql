...
;with x as (select u.UserID,u.NameLast,u.NameFirst,u.UserName,u.Code,r.Display as [Role],count(distinct CAST(asp.Expired AS Date)) as [# of LogIns],u.LastLoginDate, sc.[Name] as [School]
...
SELECT usage.UserID, usage.NameLast, usage.NameFirst, usage.UserName, usage.Code, usage.[Role], COALESCE(two.n_logins, 0) AS '2022 logins', COALESCE(usage.n_logins, 0) AS '2023 logins' ,usage.Schools 
